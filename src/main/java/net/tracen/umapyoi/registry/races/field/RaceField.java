...
public record RaceField(Identifier id, Set<Distance> turfs, Set<Distance> dirts, Set<Distance> synthetics) implements Comparable<RaceField> {
    public static final Codec<RaceField> CODEC = RecordCodecBuilder.create(instance -> instance
            .group(
                    Identifier.CODEC.fieldOf("id").forGetter(RaceField::id),
    ...
    public static final ResourceKey<Registry<RaceField>> REGISTRY_KEY = ResourceKey
            .createRegistryKey(Identifier.fromNamespaceAndPath(Umapyoi.MODID, "race_field"));
