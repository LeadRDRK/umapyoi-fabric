    ...
    public void applyFactor(ItemStack soul, UmaFactorStack stack) {
        ...
        switch (statusType) {
            case SPEED -> {
                ...
                soul.update(DataComponentsTypeRegistry.UMADATA_MAX_BASIC_STATUS.get(), UmaSoulUtils.getMaxProperty(soul),
                        data -> {
                            return new UmaDataBasicStatus(data.speed() + max, data.stamina(), data.strength(),
                                    data.guts(), data.wisdom());
                        });
                soul.update(DataComponentsTypeRegistry.UMADATA_BASIC_STATUS.get(), UmaSoulUtils.getProperty(soul), data -> {
                    return new UmaDataBasicStatus(
                            Math.min(soul.get(DataComponentsTypeRegistry.UMADATA_MAX_BASIC_STATUS.get()).speed(),
                                    data.speed() + status),
            ...
            case STAMINA -> {
                ...
                soul.update(DataComponentsTypeRegistry.UMADATA_BASIC_STATUS.get(), UmaSoulUtils.getProperty(soul), data -> {
                    return new UmaDataBasicStatus(data.speed(),
                            Math.min(soul.get(DataComponentsTypeRegistry.UMADATA_MAX_BASIC_STATUS.get()).stamina(),
                                    data.stamina() + status),
            ...
            case STRENGTH -> {
                ...
                soul.update(DataComponentsTypeRegistry.UMADATA_BASIC_STATUS.get(), UmaSoulUtils.getProperty(soul), data -> {
                    return new UmaDataBasicStatus(data.speed(), data.stamina(),
                            Math.min(soul.get(DataComponentsTypeRegistry.UMADATA_MAX_BASIC_STATUS.get()).strength(),
            ...
            case GUTS -> {
                ...
                soul.update(DataComponentsTypeRegistry.UMADATA_BASIC_STATUS.get(), UmaSoulUtils.getProperty(soul), data -> {
                    return new UmaDataBasicStatus(data.speed(), data.stamina(), data.strength(),
                            Math.min(soul.get(DataComponentsTypeRegistry.UMADATA_MAX_BASIC_STATUS.get()).guts(),
                                    data.guts() + status),
                            data.wisdom());
                });
            }
            case WISDOM -> {
                ...
                soul.update(DataComponentsTypeRegistry.UMADATA_MAX_BASIC_STATUS.get(), UmaSoulUtils.getMaxProperty(soul),
                        data -> {
                            return new UmaDataBasicStatus(data.speed(), data.stamina(), data.strength(), data.guts(),
                                    Math.min(soul.get(DataComponentsTypeRegistry.UMADATA_MAX_BASIC_STATUS.get()).wisdom(),
                                            data.wisdom() + status));
                        });
            }
        }
