...
public class UmaData {
    ...
    public UmaData(Identifier identifier, GachaRanking ranking, int[] property, int[] maxProperty,
                   int[] propertyRate, Identifier uniqueSkill, Aptitude[] surfaceAptitude, Aptitude[] distanceAptitude, Position position) {
    ...
    public static UmaData createNewUmamusume(String name, GachaRanking ranking, int[] rate, Aptitude[] aptitudes, Position pos) {
        ...
        return new UmaData(Identifier.fromNamespaceAndPath(Umapyoi.MODID, name), ranking, DEFAULT_PROPERTY,
                DEFAULT_MAX_PROPERTY, rate, Identifier.fromNamespaceAndPath(Umapyoi.MODID, "basic_pace"), surfaceAptitude, distanceAptitude, pos);
    }
}
